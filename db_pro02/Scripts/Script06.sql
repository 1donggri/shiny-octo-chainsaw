...
/*
 * 1. EMPLOYEES 테이블을 조회할 때 JOB_ID, DEPARTMENT_ID 컬럼의 ID 가 아닌
 * 	  식별 가능한 이름으로 조회 될 수 있도록 DEPARTMENTS, JOBS 테이블을 조인
 * 	  (서브쿼리도 사용 가능하기 떄문에 서브쿼리로도 만들어 본다.)
 */
...
SELECT (SELECT DECODE(STATE_PROVINCE, NULL, '', CONCAT(STATE_PROVINCE, ' ')) || CITY || ' ' || STREET_ADDRESS
		FROM LOCATIONS L WHERE LOCATION_ID = T.LOCATION_ID) AS 지역
	 , EMP_COUNT AS 사원수
