...
SELECT DEPARTMENT_ID AS 부서
	...
	,  CASE WHEN GROUPING(DEPARTMENT_ID) = 0 AND GROUPING(EXTRACT(YEAR FROM HIRE_DATE)) = 0 THEN '부서별/년도별'
			WHEN GROUPING(DEPARTMENT_ID) = 0 AND GROUPING(EXTRACT(YEAR FROM HIRE_DATE)) = 1 THEN '부서별'
	  		...
	  		WHEN GROUPING(DEPARTMENT_ID) = 1 AND GROUPING(EXTRACT(YEAR FROM HIRE_DATE)) = 1 THEN '전체(층)'
		END AS 그룹구분
	FROM EMPLOYEES
	WHERE DEPARTMENT_ID IS NOT NULL
...
SELECT FIRST_NAME || ' ' || LAST_NAME AS 직원명
	,  JOB_ID AS 직급코드
	,  SALARY AS 급여
	,  DECODE(JOB_ID, 'SH_CLERK', SALARY * (1 + 0.08), 'AD_ASST', SALARY * (1 + 0.07), 'MK_MAN', SALARY * (1 + 0.05), SALARY * (1 + 0.03)) AS 인상급여
	FROM EMPLOYEES;
