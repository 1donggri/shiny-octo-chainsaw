...
function f4() {
	var name = document.getElementById("id_name2");
	var value = document.getElementById("id_value");
	var res4 = document.getElementById("res4");
	
	if(!name.value) {
		res4.innerHTML = "속성명을 입력해주세요"
		name.focus();
		return;
	}
	
	if(!value.value) {
		res4.innerHTML = "속성값을 입력해주세요"
		count.focus();
		return;
	}
...
function f5() {
	...
	if(!width.value) {
		res5.innerHTML = "너비를 입력해주세요"
		width.focus();
		return;
	}
	
	if(!height.value) {
		res5.innerHTML = "높이를 입력해주세요"
		height.focus();
		return;
	}
	
	if(!color.value) {
		res5.innerHTML = "색상을 입력해주세요"
		color.focus();
		return;
	}
